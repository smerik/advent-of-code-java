...
class BingoSubsystemTest {
    ...
    private static final List<String> EXAMPLE_INPUT_PART_01 = List.of(
    ...
    @Test
    void testBingoSubsystem() {
        final BingoSubsystem bingo = new BingoSubsystem(EXAMPLE_INPUT_PART_01);

        assertEquals(27, bingo.getRandomNumbers().size());
        assertEquals(7, bingo.getRandomNumbers().peek());

        assertEquals(3, bingo.getBingoCards().size());
    }

    @Test
    void testPlayUntilBingo() {
        final BingoSubsystem bingo = new BingoSubsystem(EXAMPLE_INPUT_PART_01);
        final BingoCard card = bingo.playUntilBingo();
        assertEquals(10, bingo.getRandomNumbers().peek());
        assertNotNull(card);
